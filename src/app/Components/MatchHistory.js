...
export default function MatchHistory() {
  ...
  const fetchMatchHistory = async () => {
    ...
    try {
      const response = await fetch(`/api/auth/matchhistory?refereeId=${refereeId}`);
      const data = await response.json();

      if (response.ok) {
        setMatches(data.matches || []);
        setSuccess('Match history loaded successfully');
      } else {
        setError(data.message || 'Failed to load match history');
      }
    } catch {
      ...
      {filteredMatches.length > 0 ? (
        <div className="space-y-3 max-h-60 overflow-y-auto mt-4">
          {filteredMatches.map((match) => (
            ...
            >
              <h2 className="text-md font-semibold flex justify-between items-center">
